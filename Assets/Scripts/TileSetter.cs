...
public class TileSetter : MonoBehaviour
{
        ...
        for (int x = size.xMin; x <= size.xMax; x++)
        {
            for (int y = size.yMin; y <= size.yMax; y++)
            {
                ...
                if (t != null)
                {
                    ...
                    {
                        int surround = 0;
                        surround |= CheckTile(x - 1, y + 0) ? (1 << 7) : 0;
                        surround |= CheckTile(x + 1, y + 0) ? (1 << 6) : 0;
                        surround |= CheckTile(x + 0, y - 1) ? (1 << 5) : 0;
                        surround |= CheckTile(x + 0, y + 1) ? (1 << 4) : 0;
                        surround |= CheckTile(x - 1, y - 1) ? (1 << 3) : 0;
                        surround |= CheckTile(x + 1, y + 1) ? (1 << 2) : 0;
                        surround |= CheckTile(x + 1, y - 1) ? (1 << 1) : 0;
                        surround |= CheckTile(x - 1, y + 1) ? (1 << 0) : 0;
                    ...
                    {
                        tiles.SetTile(new Vector3Int(x, y, 0), null);
                        Instantiate(CoinPrefab, tiles.CellToWorld(new Vector3Int(x, y, 0)) + new Vector3(0.5f, 0.5f), Quaternion.identity);
                    }
                }
            }
        }
    ...
    private bool CheckTile(int x, int y)
    {
        ...
        if (t == null)
            return false;
        return !Collectibles.Contains(t.name);
    }
}